...
- 🔵 **In-cell formulas** — `=B2+B3`, `=SUM(B2:D2)`, refs/ranges/`+−×÷`/SUM·AVG·MIN·MAX·COUNT; view shows the result, edit shows the formula
- 🔵 **Formulas persist** save→reopen (raw sheet stored in the model; chart data derived from it) *(new)*
- 🔵 **Hide/fold** helper rows & columns — drop from view + chart, still feed formulas *(new)*
- ⬜ Fill-handle (drag-to-fill) — the remaining datasheet piece
- ⚠️ Editing a series color while rows are hidden may mis-target; structural edits clear hidden flags (documented)
