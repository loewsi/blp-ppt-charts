...
export function layoutBarColumn(model: ChartModel): Primitive[] {
  ...
  function emitRect(catStart: number, thick: number, v0: number, v1: number, fill: string) {
    if (isColumn) {
      const x = plotLeft + catStart;
      const baseline = plotTop + plotH;
      const y = baseline - v1 * valScale;
      const h = (v1 - v0) * valScale;
      prims.push({ kind: "rect", x, y, w: thick, h, fill });
      return { cx: x + thick / 2, cy: y + h / 2 };
    }
    const y = plotTop + catStart;
    const x = plotLeft + v0 * valScale;
    const w = (v1 - v0) * valScale;
    prims.push({ kind: "rect", x, y, w, h: thick, fill });
    return { cx: x + w / 2, cy: y + thick / 2 };
  }

  function pushCenteredLabel(cx: number, cy: number, thick: number, text: string, size: number) {
    const w = isColumn ? thick : 64;
    prims.push({ kind: "text", x: cx - w / 2, y: cy - 7, w, h: 14, text, color: LABEL_LIGHT, size, bold: false, align: "center" });
  }

  function pushTotal(catStart: number, thick: number, stackPx: number, text: string) {
    if (isColumn) {
      const y = plotTop + plotH - stackPx - 18;
      prims.push({ kind: "text", x: plotLeft + catStart - 8, y, w: thick + 16, h: 16, text, color: LABEL_DARK, size: 10, bold: true, align: "center" });
    } else {
      prims.push({ kind: "text", x: plotLeft + stackPx + 4, y: plotTop + catStart + thick / 2 - 8, w: PAD_MAIN + 26, h: 16, text, color: LABEL_DARK, size: 10, bold: true, align: "left" });
    }
  }

  function pushCategoryLabel(catStart: number, thick: number, text: string) {
    if (isColumn) {
      prims.push({ kind: "text", x: plotLeft + catStart - (slot - thick) / 2, y: plotTop + plotH + 3, w: slot, h: 16, text, color: LABEL_DARK, size: 9, bold: false, align: "center" });
    } else {
      prims.push({ kind: "text", x: box.left, y: plotTop + catStart + thick / 2 - 8, w: PAD_CROSS - 3, h: 16, text, color: LABEL_DARK, size: 9, bold: false, align: "right" });
    }
  }
    ...
    if (stacked) {
      ...
      for (let si = 0; si < nSer; si++) {
        ...
        if (opt.showValueLabels && v * valScale >= MIN_SEG_FOR_LABEL) {
          ...
          if (text) pushCenteredLabel(r.cx, r.cy, catThick, text, 9);
        }
        cum += v;
      }
      if (opt.showTotals && !norm100) {
        const text = formatNumber(total, nf);
        if (text) pushTotal(slotStart, catThick, total * valScale, text);
      }
    } else {
      ...
      for (let si = 0; si < nSer; si++) {
        ...
        if (opt.showValueLabels && raw * valScale >= MIN_SEG_FOR_LABEL) {
          ...
          if (text) pushCenteredLabel(r.cx, r.cy, laneThick, text, 8);
        }
      }
    }

    pushCategoryLabel(slotStart, catThick, data.categories[ci]);
  }

  // Baseline along the category axis.
  if (isColumn) {
    const baseline = plotTop + plotH;
    prims.push({ kind: "line", x1: plotLeft, y1: baseline, x2: plotLeft + plotW, y2: baseline, color: AXIS_COLOR, weight: 1 });
  } else {
    prims.push({ kind: "line", x1: plotLeft, y1: plotTop, x2: plotLeft, y2: plotTop + plotH, color: AXIS_COLOR, weight: 1 });
  }
